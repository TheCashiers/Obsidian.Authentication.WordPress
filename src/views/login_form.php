<?php          
    ...
    foreach($servers as $server)
    {
        ...
        {
        ?>
            <p><a class="button button-primary button-large" href="<?php echo(home_url()."/obsidian-auth/auth?server_name=".$server->server_name."&action=login");?>" style="margin-bottom:16px;float:none;width:100%;text-align:center;"><?php printf(__("Login with %s","obsidian-auth"),$server->server_name); ?></a></p>
        <?php
        }
    }
?>